...
Nn = 100; L = 2*Nn*max(I, D)+1;    % FIR length
h = filter_helper(I, D);
...
tic;
[y_extra2, Fs_extra2] = transformed_multi_phase(y, Fs, h, D);
disp('myDownsample then Muti-phase FIR:'); toc; fprintf('\n');

% Q6: check time domain & frequecy domain features
all_series = {y1, y2, y3, y4, y_extra2}; all_fs = {Fs1, Fs2, Fs3, Fs4, Fs_extra2};
% time domain
